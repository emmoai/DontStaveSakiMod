...
function ItemValues:On_ItemValue_Changed(player, item, keydown, ctrlkeydown, DFV_FUELEDSET, show_damage, show_uses, show_temp, show_insul, show_acond, show_adef, show_atype)
	self.item = item
	...
	if self.item.components.perishable and realfood and ctrlkeydown then
			...
			if modifier ~= 0 then
				...
				if item.components.cookable and keydown then
					...
					self.staleremainingtime =  math.floor((self.item.components.perishable.perishremainingtime/ TUNING.TOTAL_DAY_TIME / modifier - 0.5 * self.item.components.perishable.perishtime/ TUNING.TOTAL_DAY_TIME / modifier )* 10 + 0.5) / 10
					self.spoilremainingtime =  math.floor((self.item.components.perishable.perishremainingtime/ TUNING.TOTAL_DAY_TIME / modifier - 0.2 * self.item.components.perishable.perishtime/ TUNING.TOTAL_DAY_TIME / modifier )* 10 + 0.5) / 10
				else
					...
					self.staleremainingtime =  math.floor((item.components.perishable.perishremainingtime/ TUNING.TOTAL_DAY_TIME / modifier - 0.5 * item.components.perishable.perishtime/ TUNING.TOTAL_DAY_TIME / modifier )* 10 + 0.5) / 10
					self.spoilremainingtime =  math.floor((item.components.perishable.perishremainingtime/ TUNING.TOTAL_DAY_TIME / modifier - 0.2 * item.components.perishable.perishtime/ TUNING.TOTAL_DAY_TIME / modifier )* 10 + 0.5) / 10
				end	 			 
			   ...
			   if self.perishremainingtime < 0.1 then
			    ...
			    elseif strings.DFV_LANG ~= "RU" then
					self.string = self.string.."\n" .. strings.DFV_PERISHIN .. " " .. self.perishremainingtime .. " " .. strings.DFV_SPOILDAY
			    else
			    ...
			    elseif strings.DFV_LANG ~= "RU" then
					self.string = self.string.."\n" .. strings.DFV_SPOILIN .. " " .. self.spoilremainingtime .. " " .. strings.DFV_SPOILDAY
			    else
			    ...
			    elseif strings.DFV_LANG ~= "RU" then
					self.string = self.string.."\n" .. strings.DFV_STALEIN .. " " .. self.staleremainingtime .. " " .. strings.DFV_SPOILDAY
			    else
	...
	if item.components.weapon ~= nil and (show_damage or keydown) then
			...
			if item.components.armor ~= nil then
				if item.components.armor.condition ~= nil and (show_acond or keydown) then
					self.string = self.string .. "\n" .. "防具血量: " .. math.floor(item.components.armor.condition)
				end
			...
			if item.components.insulator ~= nil and (show_insul or keydown) then
				self.string = self.string .. "\n" .. math.floor(item.components.insulator.insulation) .. " 隔离"
			end
...
function ItemValues:SpawnPrefabAsServerAndRemoveFromWorld(name)
