...
local function dig_up(inst, chopper)
    ...
    if inst.components.pickable and inst.components.lootdropper then
        ...
        if inst.components.pickable:IsBarren() or inst:HasTag("withered") then
            ...
            if inst.components.pickable and inst.components.pickable:CanBePicked() then
                inst.components.lootdropper:SpawnLootPrefab("berrygr")
            end
...
local function createbush(bushname)
	...
	local prefabs =
	{
        ...
        inst.MiniMapEntity:SetIcon("berrygree.tex")
        ...
        inst.components.inspectable.nameoverride = "berrygree"
...
return createbush("berrygree")--, createbush("berryblu2") 
