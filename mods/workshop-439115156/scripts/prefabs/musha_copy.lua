...
inst:RemoveEventCallback("onhitother", on_hitLightnings_8)
 ...
 local function Lightnings_level_8(inst)
	...
	inst.strength = "valkyrie"
		...
		--frameshield_1(inst)
		--inst.components.health:SetAbsorptionAmount(0)
			scheduler:ExecuteInTime(4, function() inst.components.talker:Say("Valkyrie !") SpawnPrefab("sparks").Transform:SetPosition(inst:GetPosition():Get())  
...
---level 30 (healeh:255, sanity:470)
	inst.components.health.maxhealth = math.ceil (80 + level* 0.025)
	inst.components.sanity.max = math.ceil (120 + level* 0.05)
