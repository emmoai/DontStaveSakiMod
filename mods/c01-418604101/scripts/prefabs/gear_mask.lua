...
local function healowner(inst, owner)
    if (owner.components.health and owner.components.health:IsHurt()) then
        owner.components.health:DoDelta(1,false,"redamulet")
    end
end
...
local function onequip(inst, owner)
	...
	inst.task = inst:DoPeriodicTask(3+math.random()*0.1, function() healowner(inst, owner) end)
	...
	if not owner.components.sanityaura then
