local assets=
{
	Asset("ANIM", "anim/cacao_tree.zip"),
	
	Asset("ATLAS", "images/inventoryimages/cacao_tree.xml"),	
	Asset("IMAGE", "images/inventoryimages/cacao_tree.tex"),		
}


local prefabs = {"cacao_tree","charcoal","log","twigs",}    
...
local function chopped(inst)
	...
	if inst.burnt then
		...
		if inst.components.pickable and inst.components.pickable.canbepicked then
			inst.components.lootdropper:SpawnLootPrefab("cacao")
		end
		setupstump(inst)
...
local function fn(Sim)
	...
	inst.MiniMapEntity:SetIcon( "cacao_tree.tex" )
    
    anim:SetBank("cave_banana_tree")
    anim:SetBuild("cacao_tree")
	...
	inst.components.pickable:SetUp("cacao", TUNING.CAVE_BANANA_GROW_TIME*2)  --- 8 days
...
return Prefab("cave/cacao_tree", fn, assets, prefabs)
