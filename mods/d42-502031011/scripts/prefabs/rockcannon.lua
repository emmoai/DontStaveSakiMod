local assets = {
	Asset("ANIM", "anim/rockcannon_empty.zip"),
	Asset("ANIM", "anim/rockcannon_loaded.zip"),
	Asset("ANIM", "anim/swap_rockcannon_empty.zip"),
	Asset("ANIM", "anim/swap_rockcannon_loaded.zip"),
	
}
	for i = 0, 10, 1 do
	table.insert(assets, Asset( "IMAGE", "images/inventoryimages/rockcannon"..tostring(i)..".tex" ))
	table.insert(assets, Asset( "ATLAS", "images/inventoryimages/rockcannon"..tostring(i)..".xml" ))
	end

	local foldername = KnownModIndex:GetModActualName("Black Rock Shooter")
	local rockdmg = GetModConfigData("rockcannondmg", foldername)
...
local function UpdateShooter(inst, stage)
	inst.Ammo = stage
	inst.components.inventoryitem.atlasname = "images/inventoryimages/rockcannon"..tostring(stage)..".xml"
	inst.components.inventoryitem:ChangeImageName("rockcannon"..tostring(stage))
	...
	if stage > 0 and stage <= 10 then
		...
		inst.components.weapon:SetDamage(rockdmg)
		inst.components.weapon:SetRange(12, 16)
		inst.components.weapon:SetProjectile("blue_shot")
...
local function AmmoLeft(inst)
	...
	owner.AnimState:OverrideSymbol("swap_object", "swap_rockcannon_loaded", "rockcannon")
...
local function fn(Sim)
	...
	inst.AnimState:SetBank("rockcannon")
    inst.AnimState:SetBuild("rockcannon_loaded")
	...
	--inst:ListenForEvent("trade", UpdateShooter)
	inst.OnSave = OnSave
	inst.OnLoad = OnLoad
...
return Prefab("common/inventory/rockcannon", fn, assets)
