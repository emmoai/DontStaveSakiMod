...
local function OnEquip(inst, owner)
    ...
    if owner:HasTag("player") then
		...
		-- KAJ:TODO:MP_SOUND - think this one is actually okay as is
        if owner == ThePlayer then
            TheWorld:PushEvent("pushdsp", { dsp = SLURPER_DSP, duration = 1 })
        end
...
local function OnUnequip(inst, owner)
	...
	if owner:HasTag("player") then
		...
		-- KAJ:TODO:MP_SOUND - think this one is actually okay as is
        if owner == ThePlayer then
            TheWorld:PushEvent("popdsp", { dsp = SLURPER_DSP, duration = .5 })
        end
